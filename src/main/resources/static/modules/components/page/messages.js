...
<div class="text-message">
    <img class="msg-item" src="./assets/img/error.svg" alt="error" width="24" height="24">
...
const options = {
    info: {
        "subject":"Info: ",
        "img":{"class":classNameChild, src:"./assets/img/info.svg", "alt":status.info, "width":size.normal, "height":size.normal}
    },
    error: {
        "subject":"Error: ",
        "img":{"class":classNameChild, src:"./assets/img/error.svg", "alt":status.error, "width":size.normal, "height":size.normal}
    },
    success: {
        "subject":"Success: ",
        "img":{"class":classNameChild, src:"./assets/img/check-in-circle.svg", "alt":status.success, "width":size.normal, "height":size.normal}
    }
};
class CustomMessage {
    /**
...
function textMessageWithSymbol (parent, status, args) {
    ...
    const div = dom.element("div", parent, {"class":classNameParent});
    const img = dom.element("img", div, options[status].img);
    const small = dom.element("small", div, {"class":classNameChild});
    dom.text("strong", small, subject ? subject : subject === "" ? "" : options[status].subject);
    dom.text("small", div, text ? text : "" , {"class":classNameChild});
    return div
}
function textMessageWithSubject (parent, dataObject) {
    const {subject, data} = dataObject || {};
    const div = dom.element("div", parent, {"class":classNameParentSubjects});
